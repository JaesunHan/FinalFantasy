...
HRESULT progressBar::init(char* frontImage, char* backImage, char* fileDirectory, float x, float y, int width, int height)
{
	...
	sprintf(strTopImageName, ".//prevProjectResource//%s//%s.bmp", fileDirectory, frontImage);
	sprintf(strBottomImageName, ".//prevProjectResource//%s//%s.bmp", fileDirectory, backImage);
